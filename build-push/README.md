...
|`ssh`               |SSH agent socket or keys to expose to the build (e.g., `default=$SSH_AUTH_SOCK`, passed via env). See the [docker/build-push-action](https://github.com/docker/build-push-action#ssh) documentation for the supported format. Required when the Dockerfile uses `RUN --mount=type=ssh` to fetch private dependencies. |No      |``                        |
|`secrets`           |Secrets to expose to the build (multiline `id=<id>,src=<path>` or `id=<id>,env=<var>`). See the [docker/build-push-action](https://github.com/docker/build-push-action#secret) documentation for the supported format. Use with `RUN --mount=type=secret` in the Dockerfile.                                          |No      |``                        |
